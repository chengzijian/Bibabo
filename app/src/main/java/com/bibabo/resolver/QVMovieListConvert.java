 ...
 * @param <T>
 */
public class QVMovieListConvert<T, F> implements Function<T, Publisher<F>> {
    @Override
    public Publisher<F> apply(@NonNull T source) throws Exception {
        List<QVMovieInfo> items = new ArrayList<>();
        ...
        Element nextPage = doc.select("div.mod_pages").first().getElementsByTag("a").last();
        String classStr = nextPage.attr("class");
        boolean hasNextPage = !classStr.contains("disabled");
        items.add(0, new QVMovieInfo(hasNextPage));
